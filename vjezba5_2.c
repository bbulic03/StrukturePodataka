...
int readFile(char filename[], position stog) {
    ...
    row = (char*)malloc(MAX_LENGTH * sizeof(char));
    ...
    row = strdup(temprow);  // Copy the contents of temprow into row

    while (strlen(row) > 0) {
        value = sscanf(row, "%lf %n", &element, &numByt);
        if (value == 0) {
            // Clear the stack only if value is 0
            deleteAll(stog);
        }
        else {
    ...
    fclose(fp);
    free(row);  // Free the allocated memory for row
